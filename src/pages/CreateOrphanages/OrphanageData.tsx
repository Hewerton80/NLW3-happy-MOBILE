...
export default function OrphanageData() {
  ...
  const { createOrphanage , isCreated} = useOrphanage();
  ...
  const handleSubmit = useCallback(() => {
    //e.preventDefault();
    const { longitude, latitude } = route.params.position
  ...
  }, [name, route, instructions, openOnWeekends, openingHours, about, images])
  ...
  async function handleSelectImages() {
    ...
    let pickerResult: any = await ImagePicker.launchImageLibraryAsync({
      allowsEditing: true,//permite o usuário dar crop, editar imagem antes de subir ela
      quality: 1,
      mediaTypes: ImagePicker.MediaTypeOptions.Images// permitir só imagens
    });
    console.log(pickerResult)
    if (pickerResult.cancelled === true) {
      return;
    }
    setImages(oldImages => [...oldImages, pickerResult.uri])
  }
  useEffect(()=>{
    isCreated && navigation.navigate('OrphanagesMap');
  },[isCreated])
